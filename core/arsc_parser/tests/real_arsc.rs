...
fn global_string_pool_has_expected_values_and_one_style_span() {
    ...
    assert_eq!(
        ...
        vec![
            "Hello World!",
            "APEX Test App",
            "APEX debug fixture — real aapt2 output",
            "Mon",
            "Tue",
            "Wed",
            "b",
    ...
    for i in 1..table.global_strings.strings.len() {
        assert!(
            table.global_strings.styles[i].is_empty(),
            "only string 0 is styled"
        );
    }
}
...
fn package_identity_and_name_pools() {
    ...
    assert_eq!(
        pkg.type_strings.strings,
        vec!["array", "bool", "color", "dimen", "integer", "string"]
    );
    assert_eq!(
    ...
    assert_eq!(
        pkg.types.len(),
        6,
        "array, bool, color, dimen, integer, string"
    );
}

fn find_type<'a>(
    pkg: &'a apex_arsc_parser::package::Package,
    name: &str,
) -> &'a apex_arsc_parser::package::ResType {
    pkg.types
        .iter()
        .find(|t| t.name == name)
...
fn simple_string_entries_resolve_through_the_global_pool() {
    ...
    let by_key_name: std::collections::HashMap<&str, &apex_arsc_parser::entry::ResourceEntry> =
        config
            .entries
            .iter()
            .map(|(_, e)| (pkg.key_name(e.key_index).unwrap(), e))
            .collect();

    let app_name = by_key_name["app_name"];
    let EntryValue::Simple(v) = &app_name.value else {
        panic!("app_name should be a simple entry")
    };
    ...
    let EntryValue::Simple(v) = &hello.value else {
        panic!("hello_label should be a simple entry")
    };
    assert_eq!(
        table.resolve_string(v.data).unwrap(),
        "APEX debug fixture — real aapt2 output"
    );

    let styled = by_key_name["styled_text"];
    let EntryValue::Simple(v) = &styled.value else {
        panic!("styled_text should be a simple entry")
    };
...
fn simple_non_string_entries_carry_the_right_data_type_and_raw_data() {
    ...
    let EntryValue::Simple(v) = &entry.value else {
        panic!()
    };
...
fn complex_array_entry_has_synthetic_item_names_and_ordered_string_values() {
    ...
    let EntryValue::Complex { parent, entries } = &entry.value else {
        panic!("string-array must be a complex entry")
    };
    assert_eq!(*parent, 0);
    ...
    let resolved: Vec<&str> = entries
        .iter()
        .map(|m| table.resolve_string(m.value.data).unwrap())
        .collect();
...
fn type_spec_flags_are_all_default_for_a_single_config_fixture() {
    ...
    for t in &pkg.types {
        assert_eq!(
            t.spec_flags.len(),
            t.configs[0].entries.len(),
            "no holes in this fixture: typeSpec entryCount matches the dense entry count"
        );
        assert!(
            t.spec_flags.iter().all(|&f| f == 0),
            "no per-config variation in this fixture"
        );
    }
}
