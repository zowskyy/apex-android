...
pub enum ArscError {
    ...
    #[error(
        "truncated resources.arsc: needed {needed} bytes at offset {offset}, buffer is {len} bytes"
    )]
    Truncated {
        offset: usize,
        needed: usize,
    ...
    #[error(
        "unexpected chunk type at offset {offset}: expected 0x{expected:04x}, got 0x{got:04x}"
    )]
    UnexpectedChunkType {
        offset: usize,
        expected: u16,
